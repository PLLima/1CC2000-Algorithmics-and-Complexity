...
# --- top-level worker: must be importable & picklable on Windows ---
import traceback

def _timeout_worker(q, func, args, kwargs):
    """Run func(*args, **kwargs) and put ('ok', result, None) or ('err', None, tb) into q."""
    try:
...
def _load_answer_key(path='answer_key.json'):
    try:
        with open(path, 'r', encoding='utf-8') as f:
            return json.load(f)  # ex: {"q1":3, "q2":2, ...}
...
def isEqual(trueAnswer, predAnswer, tolerance = TOLERANCE):
    ...
    if isinstance(trueAnswer, dict) and isinstance(predAnswer, dict):
        ...
        return True
    if isinstance(trueAnswer, set) and isinstance(predAnswer, set):
        if len(trueAnswer) != len(predAnswer): return False
        for v in trueAnswer:
            found = False
            for w in predAnswer:
...
class TimeoutFunction:
    ...
    - Uses a child process (spawn context) that is terminated on timeout.
    - Target function must be picklable (top-level or functools.partial).
    """
    def __init__(self, function, maxSeconds=None):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if not self.maxSeconds or self.maxSeconds <= 0:
            return self.function(*args, **kwargs)

        q = self._ctx.Queue()
        p = self._ctx.Process(
            target=_timeout_worker,  # <-- top-level function, picklable
            args=(q, self.function, args, kwargs),
            daemon=False,
        )

        start = timer()
        p.start()
        p.join(self.maxSeconds)
        elapsed = timer() - start

        if p.is_alive():
            # Timeout: kill child and clean up
            p.terminate()
            p.join()
            try:
                q.close()
                ...
                pass
            raise TimeoutFunctionException(f"Function execution exceeded the timeout ({self.maxSeconds}s).")

        # Child exited: fetch result or error
        try:
            status, res, tb = q.get_nowait()
        except Exception:
            raise TimeoutFunctionException("Function terminated without result.")
        finally:
            try:
                q.close()
            except Exception:
        ...
        if status == "ok":
            ...
            raise TimeoutFunctionException(f"Function raised in child process:\n{tb}")        
class Part:
    def __init__(self, number, gradeFunc, maxPoints, maxSeconds, extraCredit, description, basic, counts=True):
        ...
        self.basic = basic
        self.counts = counts  # Whether this part counts towards the final grade
        # Grading the part
        self.points = 0
...
class Grader:
    def __init__(self, args=sys.argv):
        ...
        self.submission = None # Store the submission module
        ...
        self.printer = pprint.PrettyPrinter(width=41, compact=True)
    ...
    def addUtilityPart(self, number, gradeFunc, maxSeconds=defaultMaxSeconds, description=""):
        """
        Add a 'utility', 'neutral' part (executed, displayed, but not scored).
        Ideal for benchmarks / preprocessing / diagnostics.
        """
        self.assertNewNumber(number)
        # maxPoints=0 and counts=False: excluded from totals, but executed normally
        part = Part(number, gradeFunc, 0, maxSeconds, False, description, basic=True, counts=False)
        self.parts.append(part)

    # helper: récupère la réponse de la soumission
    def _get_submission_answer(self, number):
        if self.submission is None:
            raise RuntimeError("Submission not loaded: call grader.load('<module>') before adding/using open parts.")
        # Autorise 'q2' ou '2' côté tests
        attr = number if number.startswith('q') else f"q{number}"
        ans = getattr(self.submission, attr, None)
        if ans is None:
            raise AttributeError(f"Submission has no attribute '{attr}'")
        return ans() if callable(ans) else ans

    def _key_entry(self, number):
        """Retourne l'entrée brute de answer_key pour 'qN' ou 'N'."""
        k = number if str(number).startswith('q') else f"q{number}"
        if ANSWER_KEY is None:
            return None
        return ANSWER_KEY.get(k, ANSWER_KEY.get(str(number)))

    def _extract_correct_and_choices(self, number):
        """
        Retourne (correct, choices) à partir de l'entry de answer_key.
        Formats supportés:
        - int -> (correct=int, choices=None)
        - {"correct": int, "choices": int}
        - {"choices": int}
        """
        entry = self._key_entry(number)
        correct = None
        choices = None
        ...
        # Valeur par défaut raisonnable si non précisé :
        if choices is None:
            choices = 4  # change à 3/5 si ton QCM a une autre convention par défaut
        return correct, choices

    def addOpenPart(self, number, maxPoints=1, extraCredit=False, description=""):
        self.assertNewNumber(number)
        correct, choices = self._extract_correct_and_choices(number)

        def _grade():
            ans = self._get_submission_answer(number)
            # 1) validation de domaine (sans divulguer la bonne réponse)
            self.requireIsTrue(
                isinstance(ans, int),
                f"Answer to question {number} must be an integer."
            )
            self.requireIsTrue(
                1 <= ans <= choices,
                f"You must answer question {number} with an integer between 1 and {choices}."
            )

            # 2) message neutre côté étudiant
            self.addMessage(f'Your answer to question {number} is: {ans}')
            self.addMessage('It will be graded after your submission.')

            # 3) correction (côté staff/serveur si 'correct' présent)
            if correct is not None:
                self.requireIsEqual(ans, correct)

        part = Part(number, _grade, maxPoints, None, extraCredit, description, basic=True)
        self.parts.append(part)
    ...
    def load(self, moduleName):
        try:
            ...
            self.submission = mod        # <--- NEW: stocker la soumission
            return mod
    ...
    def gradePart(self, part):
        ...
        if not part.counts:
            displayPoints = 'n/a (utility)'
        elif part.is_hidden() and not self.useSolution:
    ...
    def getSelectedParts(self):
        ...
        for part in self.parts:
            if self.params.q and self.params.q != part.number:
                continue
            elif self.params.all:
                if part.counts:
                    parts.append(part)
                else:
                    continue
            elif self.params.q == part.number:
                parts.append(part)
                ...
                parts.append(part)
            elif self.mode == ALL_MODE and part.counts:
                parts.append(part)
            else:
    ...
    def grade(self):
        ...
        if not self.params.summary and not self.fatalError:
            ...
            # Count only the parts that "count"
            scoringParts = [p for p in activeParts if p.counts]

            totalPoints = sum(part.points for part in scoringParts if not part.extraCredit)
            totalPoints = sum(part.points for part in scoringParts if not part.extraCredit)
            extraCredit = sum(part.points for part in scoringParts if part.extraCredit)
            maxTotalPoints = sum(part.maxPoints for part in scoringParts if not part.extraCredit)
            maxExtraCredit = sum(part.maxPoints for part in scoringParts if part.extraCredit)
    ...
    def requireIsLessThanOrEqual(self, lessThanQuantity, predAnswer ):
        if predAnswer <= lessThanQuantity:
            return self.assignFullCredit()
        else:
            return self.fail("Expected to be <= %f, but got %f" % (lessThanQuantity, predAnswer) )
    ...
    def requireIsGreaterThanOrEqual(self, greaterThanQuantity, predAnswer ):
        if predAnswer >= greaterThanQuantity:
            return self.assignFullCredit()
        else:
            return self.fail("Expected to be >= %f, but got %f" %
    ...
    def checkOpenAnswer(self, question, answer, correct_answer=None):
        assert 1 <= answer <= 4 , f"You must answer question {question} with an integer between 1 and 4"
        self.addMessage(f'Your answer to question {question} is: {answer}')
        self.addMessage(f'It will be graded after your submission.')
        if correct_answer is not None:
            self.requireIsEqual(answer, correct_answer)
        return False
